...
class DddArchitect < Thor::Group
  ...
  say_step "Vendoring Rails application at test/delivery"

  def invoke_rails_app_generator
    invoke Rails::Generators::AppGenerator,
      [ File.expand_path(delivery_path, destination_root) ]
  end
  ...
  def change_config_files
    ...
    template "rails/boot.rb", "#{delivery_path}/config/boot.rb", :force => true
    template "rails/application.rb", "#{delivery_path}/config/application.rb", :force => true
  end

  say_step "Removing unneeded files"

  def remove_uneeded_rails_files
    inside delivery_path do
  ...
  protected
    ...
    def application_definition
      @application_definition ||= begin
        contents = File.read(File.expand_path("#{delivery_path}/config/application.rb", destination_root))
        contents[(contents.index("module Delivery"))..-1]
      end
    end
